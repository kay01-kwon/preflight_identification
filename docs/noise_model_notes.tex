...
The window contains $N$ samples $v_1, \dots, v_N$ taken every $T_s$
seconds (here $T_s \approx 5$ ms, $N \approx 80$--$300$, window
length $T = NT_s \approx 0.4$--$0.7$ s). Throughout this section $v$
stands for \emph{any} such record --- the raw gyro rate $y$, a
residual, a quiet stretch; the machinery is the same for all of
them. Treat the samples as one vector
$v \in \mathbb{R}^N$. Its size is measured by the root mean square,
\begin{equation}
  \rms(v) \;=\; \sqrt{\tfrac1N \textstyle\sum_i v_i^2}
  \;=\; \frac{\|v\|}{\sqrt N},
  \tag{N1}
\end{equation}
...
\begin{equation}
  \textstyle\sum_i v_i^2 \;=\; \text{(sum of squared bin contents)}.
  \tag{N3}
\end{equation}
...
practical meaning: $\rms(v)^2$ can be split \emph{exactly} across
groups of bins.
...
records $v = v_{\rm lo} + v_{\rm hi}$ that are built from
\emph{disjoint} sets of basis directions, hence exactly orthogonal
($\sum_i v_{{\rm lo},i}\,v_{{\rm hi},i} = 0$), hence
\begin{equation}
  \rms(v)^2 \;=\; \rms(v_{\rm lo})^2 + \rms(v_{\rm hi})^2
...
$\kappa = \rms(v_{\rm lo})/\rms(v_{\rm hi})$ is known, the total
follows from the high band alone,
\begin{equation}
  \rms(v) \;=\; \rms(v_{\rm hi})\sqrt{1 + \kappa^2}.
  \tag{N5}
\end{equation}
...
From here on the symbols are specific: $y$ is the raw gyro
angular-rate record in the window (what the sensor logged),
$\hat\omega$ the fitted cosh branch, and $r$ the \emph{minimiser
residual}
\begin{equation*}
  r \;=\; y - \hat\omega
  \qquad\text{(record minus fitted branch)}
\end{equation*}
--- so $r$ is \emph{not} the sensor data itself but what remains of
it after the model's best explanation is removed: unabsorbed model
error plus disturbance. The residual has had the trend subtracted, so
its high band is leakage-free; and above $5$ Hz the subtracted branch
changed nothing, since the model has nothing there. Hence
\begin{equation}
...
\emph{Where the cutoff comes from.} $f_c = 5$ Hz is not tuned; it is
the middle of a plateau between two constraints that close in from
opposite sides. \emph{From below}, the cutoff must clear the model's
spectral home $C_2/2\pi = 0.56$--$0.90$ Hz by a wide margin, or the
high band inherits model content and the anchor stops being
model-free --- the foundation of the whole construction. There is a
second floor: the low band must keep some bins of its own for
$\kappa_q$ to exist, and with a bin spacing of $0.97$--$4.0$ Hz the
shortest windows have \emph{no} bin below $1$--$2$ Hz.
\emph{From above}, the disturbance is coloured --- its spectrum falls
with frequency --- so raising $f_c$ makes the anchor sample an ever
quieter corner of it, the same failure mode as the textbook
estimators of Sec.~7. Sweeping the deployed construction:
\begin{center}\small
\begin{tabular}{@{}rcccc@{}}
\toprule
$f_c$ [Hz] & low-band bins & $N_{\rm med}$ [$^\circ$/s] & $N_n$ &
coverage\\
\midrule
$1$  & $0$--$1$  & $0.821$ & $2.46$ & $140/140$\\
$2$  & $0$--$2$  & $0.816$ & $2.45$ & $140/140$\\
$3$  & $0$--$3$  & $0.800$ & $2.40$ & $140/140$\\
$5$  & $1$--$5$  & $0.772$ & $2.31$ & $140/140$\\
$6$  & $1$--$6$  & $0.774$ & $2.32$ & $140/140$\\
$8$  & $1$--$8$  & $0.652$ & $1.96$ & $140/140$\\
$10$ & $2$--$10$ & $0.453$ & $1.36$ & $136/140$\\
$15$ & $3$--$15$ & $0.352$ & $1.06$ & $117/140$\\
\bottomrule
\end{tabular}
\end{center}
Between $2$ and $6$ Hz the anchor barely moves ($0.77$--$0.82$) and
the cap holds everywhere --- as theory demands, since a total does
not depend on where it is split. Past $8$ Hz the coloured decay takes
over and the anchor reads progressively low. That the answer is
insensitive across the plateau is what justifies the choice: $5$ Hz
sits about six times above the model's content, with both bands still
populated.
...
what it can follow, and the rule $w \approx 2/(f_c T_s)$ makes the
window span exactly \emph{two periods} of $f_c$. The factor $2$ is
not Nyquist's --- it counts oscillations against polynomial
flexibility: a cubic has at most two interior extrema, so it can
track about one full cycle inside its window and no more. Measured
on this kernel ($w = 41$, $T_s = 9.9$ ms): the smoother's $-3$ dB
point sits at $2.6$ Hz --- almost exactly one period per window ---
and by two periods per window ($f_c = 5$ Hz) it passes only $0.20$
of the amplitude. So (record $-$ SG smooth) keeps $80\%$-plus of
everything at and above $f_c$ while the trend --- and its leakage
--- is removed; the residual partial-tracking near the edge is
covered empirically by the cross-checks below and by the
$2$--$6$ Hz cutoff plateau of Sec.~3. The same brick wall then reads
...
10^{-16}$); moving the cutoff anywhere between $2$ and $6$ Hz changes nothing
($140/140$ throughout) because a total does not depend on where it is
split --- only past $8$ Hz, where the coloured decay leaves the
anchor sampling the disturbance's quietest corner, does the
construction degrade (Sec.~3); and the anchor is insensitive
...
\toprule
$y$, $\hat\omega$, $r$, $n$ & the raw gyro rate record in the window;
      the fitted cosh branch; the residual $r = y - \hat\omega$;
      the disturbance ($v$ in Sec.~2 is any record)\\
