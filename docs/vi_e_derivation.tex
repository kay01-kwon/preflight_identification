...
the minimiser as $t_c = t_c^* + \delta t_c$.
...
$3.3\times10^{-4}$~rad/s when the candidate onset shifts by one sample.
...
shorter. The fitted baselines are $8.5\times10^{-4}$~rad/s in the median
and $1.68\times10^{-2}$ at worst over the $140$ runs; bounding
$|C_0-b|$ by $|C_0|$ itself, which is conservative since $C_0$ estimates
$b$ from the pre-onset samples, that is $0.010$~mm of offset in the
median and $0.196$~mm at worst. The median is negligible against the
$0.40$~mm the box guarantees; the worst case is not, and it is reported
with the realised budget rather than buried here.
...
it is the manuscript's (98)--(101) written with $\chi$ named.
...
Substitute the Duhamel integral (D.2) into (D.12) and exchange the order
...
\item $\int_c^T w = 1$, exactly. This is not a numerical coincidence
  ...
  bare. Integrate the numerator of (D.13) over $s$ and exchange the
  ...
  constant survives. \texttt{error\_budget.py -{}-check} evaluates (D.13)
