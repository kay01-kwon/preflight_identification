...
\textbf{Why $n_{\rm eff}$ increases.} This is the one place a power
series is used, and only for its sign pattern --- no coefficient is
evaluated.

\emph{Lemma.} Let $f(u) = \sum_{n\ge0}a_nu^n$ converge for all $u\ge0$
with every $a_n\ge0$, and let $f$ not be a single monomial. Then
$n_{\rm eff} = uf'/f$ is strictly increasing on $(0,\infty)$.

\emph{Proof.} For $u>0$ put $p_n = a_nu^n/f(u)$. Each $p_n\ge0$ and
$\sum_np_n = 1$, so the $p_n$ are a probability distribution over the
exponents, and
\[
  \langle n\rangle = \sum_n n\,p_n = \frac{uf'}{f} = n_{\rm eff},
  \qquad
  \frac{u^2f''}{f} = \sum_n n(n-1)p_n
                   = \langle n^2\rangle - \langle n\rangle .
\]
Differentiating $n_{\rm eff}$ and grouping by these,
\[
  \frac{dn_{\rm eff}}{du}
  = \frac{f'}{f} + u\,\frac{f''f - f'^2}{f^2}
  = \frac{1}{u}\Bigl[\frac{uf'}{f} + \frac{u^2f''}{f}
                     - \Bigl(\frac{uf'}{f}\Bigr)^{\!2}\Bigr]
  = \frac{\operatorname{Var}(n)}{u} \;\ge\; 0 ,
\]
the bracket collapsing to $\langle n\rangle + \langle n^2\rangle -
\langle n\rangle - \langle n\rangle^2$. The variance vanishes only for
a point mass, i.e.\ only if $f$ is a monomial. $\square$

Raising $u$ shifts weight from low exponents to high ones, and a
variance cannot be negative: that is the whole content. Both kernels
qualify --- $\Lambda = \sum_{k\ge1}u^{2k+1}/(2k+1)!$ has non-negative
coefficients, and so do the product $\Lambda^2$ and the shift
$u\Lambda$ --- and neither is a monomial, so $n_{\rm eff}$ is strictly
increasing for both channels and $R$ is strictly decreasing. The same
distribution gives the origin values for free, since as $u\to0$ it
concentrates on the lowest exponent present, $6$ and $4$.

That route needs the series; the following one does not, and is worth
recording because it settles the same two numbers independently.
The value of $\Psi_\varphi$ at the origin is
l'H\^opital's rule, applied three times. Writing $\Psi_\varphi = N/D$
...
neither needs the series: two differentiations settle each.
...
corollary of the mean value theorem, applied three times and nowhere
supplemented.

\emph{Lemma.} If $f$ is continuous on $[0,b]$, differentiable on
$(0,b)$ with $f'\ge0$ there, and $f(0) = 0$, then $f\ge0$ on $[0,b]$.
\emph{Proof:} for $0\le s<t\le b$ the mean value theorem gives
$f(t)-f(s) = f'(\xi)(t-s)\ge0$, so $f$ is non-decreasing; taking
$s=0$ gives $f(t)\ge f(0)=0$. $\square$

Note what is \emph{not} used. No limit as $x\to\infty$ enters: the
constant $\tfrac12$ comes from the algebra of (D.10), and the
...
\textbf{Proof of (D.10).} Let $D(x)$ be the right side minus the left,
so that (D.10) is $D\ge0$. Every quantity divided by below ---
...
\emph{First application.} $f = \phi'$: both terms of
$\phi'' = \sinh x + 2x\cosh x$ are non-negative for $x\ge0$, and
$\phi'(0) = 1-1+0 = 0$. The Lemma gives $\phi'\ge0$.

\emph{Second application.} $f = \phi$: $\phi'\ge0$ by the first
application and $\phi(0) = 0$. The Lemma gives $\phi\ge0$, hence
$B\ge0$, hence $D' = \Lambda B\ge0$.

\emph{Third application.} $f = D$: $D'\ge0$ by the second application
and $D(0) = 0$. The Lemma gives $D\ge0$, which is (D.10). $\square$

Eq.~(D.11) follows the same way, its difference having $E(0) = 0$ and
$E'\ge0$; alternatively its power series has the closed general
coefficient $\bigl[2^{m-1}/m + (m^2-6m+2)/3\bigr]/(m-1)!$ with
$m = 2n$, which vanishes at $m = 2, 4$ and is positive for $m\ge6$
because both terms are.

No monotonicity of the products is claimed or needed --- only that they
do not exceed their limits, which is what (D.10) and (D.11) say.
