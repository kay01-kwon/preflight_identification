...
\item $\int_0^T w = 1$, exactly. This is not a numerical coincidence
  ...
  bare. Integrate the numerator of (D.9) over $s$ and exchange the
  ...
  constant survives. \texttt{error\_budget.py -{}-check} evaluates (D.9)
...
\section*{6b. A sharper route: use the absorption, not the monotonicity}

Section 6 threw something away. The reduction (98) is
$\Delta M_{\rm crit} = -\langle\rho, w\rangle$, and $\rho$ there is
\emph{already} the out-of-span part, $\rho = P\rho$ with $P$ the
orthogonal projection off $\mathrm{span}\{1,\tau,\delta\varphi\}$.
An orthogonal projection is self-adjoint and idempotent, so
\begin{equation}
  \Delta M_{\rm crit} = -\langle P\rho,\,w\rangle
                      = -\langle P\rho,\,Pw\rangle
  \qquad\Longrightarrow\qquad
  |\Delta M_{\rm crit}| \;\le\; \|P\rho\|_\infty\;\|Pw\|_1 .
  \tag{D.10}
\end{equation}
No monotonicity is used --- only the absorption the reduction already
assumes. And the point is that \emph{$w$ itself lies mostly in the
span}: it is smooth, positive and slowly varying, so removing a
constant, a ramp and a $\Lambda$ leaves very little, and $\|Pw\|_1$ is
an order of magnitude below the unit mass $\int w = 1$.

\textbf{Both factors are elementary.} The weight needs no quadrature.
Writing $\sinh v\cosh(v-s) = \tfrac12\bigl[\sinh(2v-s) + \sinh s\bigr]$
and integrating,
\begin{equation}
  \int_s^{x}\!\sinh v\,\cosh(v-s)\,dv
  = \tfrac14\bigl[\cosh(2x-s) - \cosh s\bigr]
    + \tfrac12\,(x-s)\sinh s ,
  \tag{D.11}
\end{equation}
which is $w$ up to the normalisation $\int_0^x w = 1$. So the recipe,
for a given $x$, is four lines of arithmetic on a grid:
\begin{enumerate}\itemsep0pt
\item tabulate $w(s)$ from (D.11) on $s\in[0,x]$ and divide by its
  integral;
\item least-squares fit $\alpha + \beta s + \gamma\Lambda(s)$ to
  $w$, subtract, and take $\|Pw\|_1 = \int_0^x|w - \text{fit}|\,ds$;
\item do the same fit-and-subtract to $\Lambda(s)^2$ and to
  $s\Lambda(s)$, and take the sup-norm of each residual divided by its
  endpoint value --- these are $\|P\rho_\varphi\|_\infty/
  \rho_{\varphi,\max}$ and $\|P\rho_{GE}\|_\infty/\rho_{GE,\max}$;
\item multiply.
\end{enumerate}
Calling the products $K_\varphi$ and $K_{GE}$,
\begin{equation}
  \boxed{\;|\Delta M_{\rm crit}| \;\le\;
   K_\varphi(x)\,\rho_{\varphi,\max} + K_{GE}(x)\,\rho_{GE,\max},
   \qquad
   K \;=\; \frac{\|P\rho\|_\infty}{\rho_{\max}}\;\|Pw\|_1 \;}
  \tag{D.12}
\end{equation}

\textbf{What it buys, and what it costs.}
\begin{center}\small
\begin{tabular}{@{}rrrrrr@{}}
\toprule
$x$ & $\|Pw\|_1$ & $K_\varphi$ & $K_{GE}$ & $R_\varphi$ & $R_{GE}$\\
\midrule
$1.00$ & $0.0024$ & $0.0004$ & $0.0001$ & $0.1397$ & $0.1972$\\
$2.00$ & $0.0401$ & $0.0067$ & $0.0018$ & $0.1311$ & $0.1891$\\
$2.99$ & $0.1147$ & $0.0202$ & $0.0048$ & $0.1190$ & $0.1772$\\
$3.80$ & $0.1993$ & $0.0369$ & $0.0079$ & $0.1079$ & $0.1656$\\
$5.21$ & $0.3799$ & $0.0776$ & $0.0138$ & $0.0892$ & $0.1444$\\
$7.94$ & $0.7398$ & $0.1772$ & $0.0240$ & $0.0624$ & $0.1088$\\
\bottomrule
\end{tabular}
\end{center}
$K$ \emph{rises} with $x$ where $R$ falls, and the two cross near
$x\simeq5.5$. Over the operating box, where $x\le5.21$, the sharp form
wins on both channels --- narrowly on gravity, by a factor of ten on
the ground-effect channel, whose shape $s\Lambda(s)$ the span
approximates especially well because $\Lambda$ is one of its members.
Quoting $\min(K,R)$ per channel is safe and is what the evaluation
does. The cost is that (D.12) is no longer free of $x$: it must be
read at the window the run actually has, whereas (97) holds whatever
the window.

\textbf{The exact gravity remainder.} One more factor comes free.
$\rho_\varphi$ is the second-order remainder of
$G(\varphi) = -Wa\cos\varphi + Wz_{CoM}\sin\varphi$ about
$\varphi = 0$, and that remainder is elementary:
\begin{equation}
  \rho_\varphi(\varphi) = G(\varphi) - G(0) - G'(0)\varphi
  = W\bigl[\,a\,(1-\cos\varphi) + z_{CoM}(\sin\varphi - \varphi)\,\bigr],
  \tag{D.13}
\end{equation}
with $a = l_p+\lambda_{\rm off}$. The second term is negative, so the
customary Taylor bound $\tfrac12 Wa\varphi^2$ --- which is (D.13) with
$\cos$ expanded to second order and the $z_{CoM}$ term dropped
altogether --- is loose by $8$ to $14\%$ over the box. There is no
reason to pay it.
