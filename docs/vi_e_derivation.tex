...
$t_0$ to minimise
\[
  J(t_0) = \int_{t_0}^{t_{\rm end}}\!
           \bigl(y(t) - \hat\omega(t;t_0)\bigr)^2dt ,
  \qquad
  \hat\omega(t;t_0) = C_1\bigl(\cosh C_2(t-t_0) - 1\bigr) .
\]
The lower limit moves with $t_0$: the fit is applied to the post-onset
segment only. In the shifted variable $\tau = t - t_0$ that window is
$[0,T]$ with $T = t_{\rm end} - t_0 = \te$, the window Sec.~2 averages
over, so the $\rbar$ that appears below is the same $\rbar$.

Write the data as nominal plus deviation, $y = \hat\omega(t;t_0^*) +
e_\omega$ --- the perturbation is $\rho$ itself, not its average --- and
the minimiser as $t_0 = t_0^* + \delta$. Name the sensitivity
\[
  \chi(t) \;\triangleq\;
  \frac{\partial\hat\omega(t;t_0)}{\partial t_0}\bigg|_{t_0 = t_0^*}
  \;=\; -\,C_1C_2\sinh\bigl(C_2(t-t_0^*)\bigr) .
\]
...
$\hat\omega(t;t_0^*+\delta)\simeq\hat\omega(t;t_0^*) + \delta\chi$, so
the residual is $e_\omega - \delta\chi$ and
\[
  \frac{\partial J}{\partial t_0}
  = \underbrace{-\bigl(y(t_0) - \hat\omega(t_0;t_0)\bigr)^2}
              _{\text{from the moving lower limit}}
    \;-\;2\!\int_{t_0}^{t_{\rm end}}\!\bigl(e_\omega - \delta\chi\bigr)
    \chi\,dt \;=\; 0 .
\]
The boundary term is negligible at this order, for the same reason the
Duhamel boundary term of (88) was: $\hat\omega(t_0;t_0) =
C_1(\cosh 0 - 1) = 0$ and the true rate at the onset is itself
$\mathcal{O}(\delta^2)$, so the bracket is $\mathcal{O}(\delta^2)$ and
its square $\mathcal{O}(\delta^4)$. Dropping it and solving,
\[
  \delta = \frac{\langle e_\omega,\chi\rangle}{\|\chi\|^2},
  \qquad
  \langle a,b\rangle \triangleq \int_{t_0^*}^{t_{\rm end}}\!a(t)\,b(t)\,dt ,
\]
the division legitimate because $\|\chi\|^2>0$ --- $\sinh(C_2\tau)$ is
not identically zero on a window of positive length. With
$\Delta M_{\rm crit} = \dot M\delta$ and $C_1 = \dot M/\Wz$ the
...
it is the manuscript's (98)--(101) written with $\chi$ named. Minimising
$J$ numerically against a deviation of the true shape and comparing with
the formula gives ratios $1.0000$, $0.9995$, $0.9953$, $0.9556$ as the
deviation grows over four decades: the formula is the first-order term,
and the discarded boundary term does not spoil it.
