...
box of Sec.~VI-D rather than at any fitted constant: $W = 30.08$~N
(unloaded, $m = 3.067$~kg), $z_{CoM} = 0.30$~m, $a = l_p + p_{\rm off}
...
\midrule
geometry & $\Wz = Wz_{CoM}$ & $9.024$~N\,m\\
(82) lower & $J_P \ge m(z_{CoM}^2+a^2)$ & $0.3546$~kg\,m$^2$\\
(82) upper & $J_P \le J_{CoM}^{\rm CAD} + m(z_{CoM}^2+a^2)$ & $0.4083$~kg\,m$^2$\\
       & $C_2 \le \sqrt{g z_{CoM}/(z_{CoM}^2+a^2)}$ & $5.045$~s$^{-1}$\\
\midrule
tilt cap $\to\Lambda$ & $\Lambda_{\max} = \varphi_{\max}\Wz C_2/\dot M$ & $6.621$\\
(92)   & $\bar x = (6\Lambda_{\max})^{1/3}$ & $3.412$\\
exact  & solve $\sinh x - x = \Lambda_{\max}$ & $x = 2.955$\\
(93)   & $\tau_{\rm end}\le(6\varphi_{\max}J_P/\dot M)^{1/3}$ & $0.709$~s\\
(93)   & $\Delta M_{\rm win} = \dot M\tau_{\rm end}$ & $0.851$~N\,m\\
\midrule
channel& $\rho_{\varphi,\max} = \tfrac12W a\,\varphi_{\max}^2$ & $73.30$~mN\,m\\
(93)   & $\rho_{GE,\max} = |\beta_M|\Delta M_{\rm win}\varphi_{\max}$ & $5.12$~mN\,m\\
\midrule
(D.6)  & $R_\varphi(3.412)$ & $0.1132$\\
(D.7)  & $R_{GE}(3.412)$ & $0.1713$\\
(95)   & $\rbar = R_\varphi\rho_{\varphi,\max}+R_{GE}\rho_{GE,\max}$
       & $9.18$~mN\,m\\
       & (against $\rmax = 78.4$~mN\,m) & \\
\midrule
(96)   & $\Psi(\bar x) = \bar x\coth(\bar x/2)$ & $3.645$\\
(96)   & $\Psi\rbar/\Delta M_{\rm win}$ & $3.93\%$\\
(97)   & $(\tfrac12\rho_{\varphi,\max}+\rho_{GE,\max})/\Delta M_{\rm win}$
       & $4.91\%$\\
(97b)  & $\rho_{\varphi,\max}/7 + \rho_{GE,\max}/5$ & $11.50$~mN\,m\\
       & $\div\,W$ & $0.382$~mm\\
\midrule
(D.3)  & envelope only, $\Psi\rmax/\Delta M_{\rm win}$ & $33.6\%$\\
\bottomrule
\end{tabular}
\end{center}
