...
$t_c$ to minimise
\[
  J(t_c) = \int_{t_c}^{t_{\rm end}}\!
           \bigl(y(t) - \hat\omega(t;t_c)\bigr)^2dt ,
  \qquad
  \hat\omega(t;t_c) = C_1\bigl(\cosh C_2(t-t_c) - 1\bigr) .
\]
The lower limit moves with $t_c$: the fit is applied to the post-onset
segment only. In the shifted variable $\tau = t - t_c$ that window is
$[0,T]$ with $T = t_{\rm end} - t_c = \te$, the window Sec.~2 averages
over, so the $\rbar$ that appears below is the same $\rbar$.

Write the data as nominal plus deviation, $y = \hat\omega(t;t_c^*) +
e_\omega$ --- the perturbation is $\rho$ itself, not its average --- and
the minimiser as $t_c = t_c^* + \delta t_c$. Name the sensitivity
\[
  \chi(t) \;\triangleq\;
...
$\hat\omega(t;t_c^*+\delta t_c)\simeq\hat\omega(t;t_c^*) + \delta t_c\,\chi$, so
the residual is $e_\omega - \delta t_c\,\chi$ and
\[
  \frac{\partial J}{\partial t_c}
  = \underbrace{-\bigl(y(t_c) - \hat\omega(t_c;t_c)\bigr)^2}
...
Duhamel boundary term of (88) was: $\hat\omega(t_c;t_c) =
C_1(\cosh 0 - 1) = 0$ and the true rate at the onset is itself
$\mathcal{O}\bigl((\delta t_c)^2\bigr)$, so the bracket is $\mathcal{O}\bigl((\delta t_c)^2\bigr)$ and
its square $\mathcal{O}\bigl((\delta t_c)^4\bigr)$. Dropping it and solving,
\[
...
domain):
\[
  \int_c^{T}\!\!\sinh(C_2\tau)\!\int_c^{\tau}\!\!\dot h_\varphi(\tau-s)
    \rho(s)\,ds\,d\tau
  = \int_c^{T}\!\!\rho(s)
...
\begin{equation}
  \Delta M_{\rm crit} = -\!\int_c^{T}\!\rho(s)\,w(s)\,ds,
  \qquad
...
\item $\int_c^T w = 1$, exactly. This is not a numerical coincidence
  ...
  either way, so
  \[
    \int_c^{T}\!\!\int_s^{T}\!\sinh(C_2\tau)\cosh\bigl(C_2(\tau-s)\bigr)
      d\tau\,ds
    = \int_c^{T}\!\sinh(C_2\tau)
      \left[\int_c^{\tau}\!\cosh\bigl(C_2(\tau-s)\bigr)ds\right]d\tau .
  \]
  The inner integral is elementary --- substitute $u = \tau - s$, whose
  sign flip is cancelled by the reversed limits ---
  \[
    \int_c^{\tau}\!\cosh\bigl(C_2(\tau-s)\bigr)ds
    = \int_c^{\tau}\!\cosh(C_2u)\,du
    = \frac{\sinh(C_2\tau)}{C_2},
  \]
  and putting it back reproduces the very norm that sits in the
  denominator of (D.12),
  \[
    \frac{1}{C_2}\int_c^{T}\!\sinh^2(C_2\tau)\,d\tau
...
\[
  \Bigl|\int_c^T\!\rho\,w\Bigr|
  \;\le\; \frac{1}{T}\int_c^T\!\rho \int_c^T\! w
  \;=\; \rbar\cdot 1 \;=\; \rbar ,
\]
and substituting (95),
\begin{equation}
...
(D.6a) & $R = \int_c^1 f(xt)/f(x)dt$, $n_{\rm eff}\uparrow$
