...
\begin{equation}
  ...
  \tag{98}
\end{equation}
with $\kappa_b$ the largest quiet-segment band ratio
$\rms({<}f_c)/\rms({>}f_c)$ of the campaign
($\kappa_b = 1.31$), measured on quiet stretches of the same
records, entirely outside the windows being bounded. (Sec.~VI-D
also measures the in-window enhancement $s_{\rm med} = 1.60$, the
median excess of the in-window over the quiet band ratio; it is a
diagnostic of where the low-frequency content comes from and does
not enter the bound.) In
...
\kappa_b^2} = 2.47^\circ$/s in (98)) all $140$ runs
...
platforms differ in. The used fraction rises monotonically with the
ramp rate (mean $0.09$ to $0.55$ in simulation, $0.13$ to $0.30$ on
hardware): the cap charges the envelope of $e_\omega$, whose window
RMS $\sqrt{B(x)/x}$ grows with the design window $x(\Mdot)$, while
the measured residual is rate-flat --- the onset freedom absorbs
the growing mode whatever the window length. The certificate is
therefore deliberately conservative at slow ramps and tightest at
the fast ramps, where the window is shortest and the identification
hardest; tightening the slow-ramp side would require charging the
post-absorption remainder, the quantity the Remark below shows no
platform measurement can isolate.
