...
$\theta = (C_1^*, C_2^*, t^*, C)$ free, minimising the objective of
(92). The nominal solution is itself a member of this family: the
choice $\theta_0 = (C_1, C_2, t_{\rm on}, b)$ reproduces
$\omega_{\rm nom} + b$ exactly. A minimum never exceeds the value at
one feasible point, so the fit cannot lose to this \emph{witness},
and
\begin{equation}
...
no calibration on either side. In simulation ($\varphi_{\max} =
5^\circ$, envelope alone) $301$ of $308$ runs sit inside, the
seven exceedances all at the two fastest ramps: three marginal
(within $7\%$) and four on the two cases whose injected offsets lie
at or beyond the design rectangle assumed by $\rbar$. On hardware
($\varphi_{\max} = 8^\circ$, envelope plus (98)) $134$ of $140$
runs sit inside; the six exceedances carry disturbance content
outside the modelled channels --- gusts and contact irregularities
at the pivot for which no budget term is reserved. The median used
fraction is $0.47$--$0.48$ on both campaigns: one construction,
validated on both platforms, the panels differing exactly in the
channels the platforms differ in.

\paragraph*{Remark}
An earlier version of this analysis bounded $\delta e_\omega$
itself through the Dirichlet response of the forcing channels
[the former (98)--(108)]. Measured directly, that cap bounds the
in-family forcing response but not the realised remainder, whose
out-of-family content is amplified by the same $\sinh$ growth the
envelope charges for; it is therefore omitted, and the certificate
on the residual is (99) alone. The decomposition (95)--(97b)
retains its two roles: the a-priori ceiling on the onset shift
consumed by Sec.~VI-F, and the explanation of the residual's
rate-flatness.
