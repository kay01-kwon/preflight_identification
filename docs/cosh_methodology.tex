...
$N_{\mathrm{pre}},\,N_{\mathrm{post}}$ & sweep margins & $30$, $8$ & baseline / fit minima \\
\bottomrule
\end{tabular}
...
condition for any onset position ($30$ pre-onset for the baseline and
sweep, $8$ post-onset for the fit).
...
\label{alg:cosh}
\begin{algorithmic}[1]
\Require windowed samples $\{(t_i,\omega_i)\}_{i=1}^{N}$; moment $M(t)$;
...
\For{$j = N_{\mathrm{pre}}, \dots, N - N_{\mathrm{post}}$}
  \Comment{exhaustive sweep of the single unknown}
  \State $C \gets \operatorname{median}\{\omega_i : i < j\}$
    \Comment{baseline by continuity}
  \State $\mathrm{cost}(j) \gets \sum_{i<j}(\omega_i - C)^2
    + \sum_{i\ge j}\bigl(\omega_i - [\,C_1(\cosh C_2 (t_i - t_j) - 1)
    + C\,]\bigr)^2$
\EndFor
...
\begin{algorithm}[h]
\caption{Rig-constant calibration by ramp invariance (coarse-to-fine)}
\label{alg:calib}
\begin{algorithmic}[1]
\Require gated runs at rates $r \in \{0.10,\dots,1.20\}$~N\,m/s, both
  directions; box $C_2 \in [3,8]$~rad/s, $K \in [0.05,0.70]$~(N\,m)$^{-1}$
\Ensure calibrated pair $(C_2^\ast, K^\ast)$
\Function{score}{$C_2, K$}
  \For{each gated run $r$}
    \State $M_{\mathrm{crit}}^{(r)} \gets$
      Algorithm~\ref{alg:cosh} with $(C_2, K)$ pinned
      \Comment{stride-3 sweep}
  \EndFor
  \State \Return $\sum_{\mathrm{dir}\in\{+,-\}}
    \mathrm{CV}\bigl(\{M_{\mathrm{crit}}^{(r)}\}_r\bigr)$
  \Comment{ramp invariance, Eq.~\eqref{eq:score}}
\EndFunction
\State \textbf{coarse:} evaluate \Call{score}{$\cdot$} on the
  $11{\times}11$ grid $C_2 = 3.0\!:\!0.5\!:\!8.0$,
  $K = 0.10\!:\!0.04\!:\!0.50$
\State $(C_2^{b}, K^{b}) \gets \arg\min$ over the coarse grid
\State \textbf{fine:} evaluate \Call{score}{$\cdot$} on a $9{\times}9$
  uniform grid over $[C_2^{b} \pm 0.5] \times [K^{b} \pm 0.04]$,
  clipped to the box \Comment{resolution $0.125$~rad/s,
  $0.01$~(N\,m)$^{-1}$}
\State $(C_2^\ast, K^\ast) \gets \arg\min$ over the fine grid
\end{algorithmic}
\end{algorithm}

\paragraph{Coarse-to-fine procedure.} Algorithm~\ref{alg:calib}
summarizes the search. The coarse scan evaluates
