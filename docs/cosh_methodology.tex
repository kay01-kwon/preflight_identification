...
can be made exact. On the fitted window the closed form is
tail-dominated: for $C_2\tau \gtrsim 1$,
\begin{equation}
C_1\bigl(\cosh(C_2\tau) - 1\bigr) \;\approx\; \tfrac{C_1}{2}\,e^{C_2\tau}
\;=\; \underbrace{\tfrac{C_1}{2}\,e^{-C_2 t_{\mathrm{on}}}}_{\textstyle A}
\; e^{C_2 t},
\end{equation}
so the amplitude and the onset enter the data only through the product
$A$: the substitution
\begin{equation}
...
\label{eq:ridge}
\end{equation}
leaves the tail invariant for every $\lambda > 0$ — an earlier onset
with a smaller amplitude trades off exactly against a later onset with
a larger one. Equivalently, the sensitivity vectors
$\partial\omega/\partial C_1 \propto \cosh(C_2\tau) - 1$ and
...
residual is nearly singular along $\delta C_1/C_1 = C_2\,\delta t$. The
degeneracy is broken only near the onset, where the response is
quadratic, $\tfrac12 C_1 C_2^2 \tau^2$, and of the order of the gyro
noise — hence a \emph{shallow ridge} in the $(C_2, K)$ plane whose
minimizer is set by noise, not physics.
