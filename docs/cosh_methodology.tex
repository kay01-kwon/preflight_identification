...
Algorithm~\ref{alg:cosh} on the run with commanded rate $r$. The pair is
selected by an exhaustive coarse scan over the physically admissible box
($C_2 \in [3,8]$~rad/s, $K \in [0.05, 0.7]$~(N\,m)$^{-1}$) followed by a
local grid refinement about the coarse minimizer; each evaluation is an
arithmetic sweep, so no iterative optimizer is involved anywhere in the
pipeline.
