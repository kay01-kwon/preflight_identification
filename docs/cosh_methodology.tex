...
$N_{\mathrm{pre}},\,N_{\mathrm{post}}$ & sweep margins & $8$, $8$ & degenerate-candidate guards \\
\bottomrule
\end{tabular}
...
samples into the window even at the fastest ramp; adding the $8$-sample
post-onset fit minimum gives $38$.
...
no-optimizer design. \emph{Sweep margins.} The margins
$N_{\mathrm{pre}} = N_{\mathrm{post}} = 8$ excise only degenerate
candidates: near $j = 1$ the baseline would be estimated from a handful
of samples, and near $j = N$ the post-onset arc is too short for the
cosh branch to be falsifiable (at $j = N$ its cost is an empty sum).
