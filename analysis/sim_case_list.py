...
REGIME = {'small': 'small offset', 'corner': 'box corner',
          'diagonal': 'roll boundary', 'shallow': 'pitch boundary',
          'loaded': 'loaded'}
...
def checklist(rows, outdir):
    ...
    # the same list as a PDF, which is what gets printed and ticked
    def esc(t):
        for a, b in (('_', r'\_'), ('%', r'\%'), ('&', r'\&')):
            t = t.replace(a, b)
        return t

    def rates_tex(label, rates):
        boxes = r' \quad '.join(f'$\\square$~{v:.2f}' for v in rates)
        return f'\\hspace*{{6mm}}{label}\\quad {boxes}\\par'
    ...
    T = [r'\documentclass[10pt]{article}',
         ...
         r'\begin{document}',
         r'\section*{Simulation run checklist}',
         f'Generated by \\texttt{{analysis/sim\\_case\\_list.py}}. '
         f'{n_abl + n_main} runs in total. Each entry is one (case, axis, '
         f'ramp rate); both tip directions are run under it, so one tick '
         f'covers two excitations. Rates in N\\,m/s.\\par\\medskip',
         f'$\\square$~\\textbf{{1. Rotor ablation, ideal rotor}} '
         f'({n_abl} runs) --- run this first\\par',
         f'$\\square$~\\textbf{{2. Main sweep, identified rotor}} '
         f'({n_main} runs)\\par\\medskip',
         r'\hrule\medskip',
         r'\subsection*{1. Rotor ablation --- ideal rotor (R0)}',
         'Compare against the same cases in the main sweep, which uses '
         'the identified second-order-plus-delay rotor (R3). Plot the '
         'offset error against the ramp rate: R0 should be flat and R3 '
         r'should not.\par\medskip']
    for cid in ABLATION_CASES:
        r = by_id[cid]
        k = rows.index(r) + 1
        T += [f'\\textbf{{S{k} (R0)}} --- ({r["x_off_mm"]:g}, '
              f'{r["y_off_mm"]:g}) mm\\par',
              rates_tex('roll', RAMP_RATES),
              rates_tex('pitch', RAMP_RATES), r'\medskip']

    T += [r'\hrule\medskip',
          r'\subsection*{2. Main sweep --- identified rotor (R3)}']
    for k, r in enumerate(rows, 1):
        rates = RAMP_RATES if r['verdict'] == 'ok' else RAMP_FAIL
        T += [f'\\textbf{{S{k}}} --- {REGIME[r["group"]]}, '
              f'({r["x_off_mm"]:g}, {r["y_off_mm"]:g}) mm, '
              f'{r["mass_kg"]} kg\\par',
              f'\\hspace*{{6mm}}\\footnotesize predicted $M_{{crit}}$: '
              f'roll {r["roll_neg"]:+.2f} / {r["roll_pos"]:+.2f}, '
              f'pitch {r["pitch_neg"]:+.2f} / {r["pitch_pos"]:+.2f}'
              + ('' if r['verdict'] == 'ok'
                 else r' --- \textbf{expected to fail}: '
                      + esc(r['verdict']))
              + r'\normalsize\par',
              rates_tex('roll', rates),
              rates_tex('pitch', rates),
              r'\hspace*{6mm}$\square$~'
              + ('identified offset within 1\\,mm of truth'
                 if r['verdict'] == 'ok' else
                 'the predicted contact lifts at zero applied moment')
              + r'\par\medskip']
    T += [r'\end{document}']
    (outdir / 'sim_checklist.tex').write_text('\n'.join(T))
    return n_abl, n_main
