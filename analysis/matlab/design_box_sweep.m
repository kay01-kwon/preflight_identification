...
[~, imx] = max(rms_b(:, end));
[~, imn] = min(rms_b(:, end));
text(a1, rates(end), hi_r(end) + 0.045*max(hi_r), sprintf(...
     ['box max: $W=%.1f$ N, $l_p{-}p_{\\mathrm{off}}=%.3f$ m, ' ...
      '$z_{CoM}=%.2f$ m'], Wv(imx), armv(imx), zv(imx)), ...
     'Interpreter', 'latex', 'FontSize', 7, 'Color', cA, ...
     'HorizontalAlignment', 'right');
text(a1, rates(end), lo_r(end) - 0.055*max(hi_r), sprintf(...
     ['box min: $W=%.1f$ N, $l_p{-}p_{\\mathrm{off}}=%.3f$ m, ' ...
      '$z_{CoM}=%.2f$ m'], Wv(imn), armv(imn), zv(imn)), ...
     'Interpreter', 'latex', 'FontSize', 7, 'Color', cA, ...
     'HorizontalAlignment', 'right');
